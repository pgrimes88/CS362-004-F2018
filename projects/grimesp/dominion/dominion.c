...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case smithy:
      return smithyFunction(handPos, state);
		
		case great_hall:
		  return great_hallFunction(handPos, state);
		
		case outpost:
		  return outpostFunction(handPos, state);
...
int smithyFunction(int handPos,struct gameState *state){
  int i;
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
...
int great_hallFunction(int handPos, struct gameState *state){
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
  if (nextPlayer > (state->numPlayers - 1)){
    nextPlayer = 0;
...
int outpostFunction(int handPos, struct gameState *state){
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
  if (nextPlayer > (state->numPlayers - 1)){
    nextPlayer = 0;
