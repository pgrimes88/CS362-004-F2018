...
int main(){
    ...
    while(1){
        ...
        int test = 0;
        for(int i = 0; i < 27; i++){
            if(game1.handCount[j] == gameOld.handCount[j] && game1.deckCount[j] == gameOld.deckCount[j]  && whoseTurn(&game1) == whoseTurn(&gameOld)){
                //
            }
            else{
                test = 1;
        ...
        if(game1.handCount[currentPlayer] == gameOld.handCount[currentPlayer]){
            test3PassCount +=1;
        }
        
        //test #4 -- deckcount was changed of current player
        if(game1.deckCount[currentPlayer] + 1 == gameOld.deckCount[currentPlayer]){
            test4PassCount +=1;
        }
        
        //test #5 -- actions was changed of current player
        if(game1.numActions == gameOld.numActions + 2){
            test5PassCount +=1;
        }
    ...
    printf("Test #3 Pass count %d/50000  (handcount was NOT changed of current player (draw 1 play 1))\n", test3PassCount);
    printf("Test #4 Pass count %d/50000  (deckcount was changed of current player - 1 card)\n", test4PassCount);
    printf("Test #5 Pass count %d/50000  (number of actions was increased by 2)\n", test5PassCount);
