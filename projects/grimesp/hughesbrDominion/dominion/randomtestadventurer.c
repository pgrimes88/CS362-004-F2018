...
int main(){
    ...
    while(1){
    	...
    	*/
    	...
    	int tempHand = game1.handCount[tempplayer];
    	//int tempDeck = game1.deckCount[currentPlayer];
        cardEffect(adventurer, 0, 0, 0, &game1, 0, &bonus);
        if(game1.handCount[tempplayer] - 2 == tempHand){
            test3PassCount +=1;
        }
        else{
            //test3PassCount +=1;
        ...
        if(game1.handCount[currentPlayer] - 2 == gameOld.handCount[currentPlayer]){
            ...
            //printf("%d   %d\n",game1.handCount[currentPlayer], gameOld.handCount[currentPlayer]);
        }
