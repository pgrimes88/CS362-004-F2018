...
{ 
  int temp2 = currentPlayer;
  int temp = state->handCount[temp2];
  //printf("\n\n%d    ",state->handCount[currentPlayer]);
  ...
  //printf("%d     ",state->handCount[currentPlayer]);
  if(state->handCount[temp2] - 2 == temp){
    //printf("true");
  }
  else{
    printf("false\n\n");
...
void card_village(int currentPlayer, struct gameState *state, int handPos)
{
  ...
  //function for the council room card
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case council_room:
     card_council_room(currentPlayer, state, handPos);
	 return 0;
    ...
    case great_hall:
      card_great_hall(currentPlayer, state, handPos);
      return 0;
...
{
	printf("\n\n%d    ",state->discardCount[currentPlayer]);
